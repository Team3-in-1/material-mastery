...
class BillService {
  ...
  getExportById = async (id: string): Promise<Bill_Export> => {
    return await axios
      .get(`${constant.BASE_URL}/bill/export/${id}`, { headers: this.hearders })
      .then((res) => {
  ...
  getImportById = async (id: string): Promise<Bill_Import> => {
    return await axios
      .get(`${constant.BASE_URL}/bill/import/${id}`, { headers: this.hearders })
      .then((res) => {
        return res.data.metadata
      })
      .catch((err) => {
  ...
  getAllExportBill = async (
    limit: number = 10,
    page: number = 1,
    sortBy: string = 'bill_date',
    isAscending: boolean = false,
  ): Promise<Bill_Export[]> => {
    return await axios
      .get(
        `${constant.BASE_URL}/bill/export/?limit=${limit}&page=${page}&sorted[]=${sortBy}&isAscending=${isAscending}`,
        { headers: this.hearders },
      )
      .then((res) => {
        return res.data.metadata
  ...
  getAllImportBill = async (
    ...
    return await axios
      .get(
        `${constant.BASE_URL}/bill/import/?limit=${limit}&page=${page}&sorted[]=${sortBy}&isAscending=${isAscending}`,
        { headers: this.hearders },
      )
      .then((res) => {
        return res.data.metadata
  ...
  getNumberOfBill = async (): Promise<NumOfBill> => {
    ...
    return await axios
      .get(`${constant.BASE_URL}/bill/number`, { headers: this.hearders })
      .then((res) => {
        return res.data.metadata
      })
      .catch((err) => {
