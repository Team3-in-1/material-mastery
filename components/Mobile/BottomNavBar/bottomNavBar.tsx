...
import { IconHome, IconCategory, IconShoppingCart, IconUserCircle } from '@tabler/icons-react'
...
function IconWithProps({ element, size, color }: PropsOfIcon) {
    return (
        <>
            {React.cloneElement(element, { size, color })}
        </>
    );
}
...
function NavButton({ icon, size, active, onClick }: navBarBtnProps) {
    ...
    return (
        <Button p='0.625rem' variant='filled' color={styleBtn.bgColor} h={`cal(${size} + 0.625rem)`} onClick={onClick}>
...
export default function BottomNavBar() {
    const [activeBtn, setActiveBtn] = useState<boolean[]>(Array.from({ length: listButtonElement.length }, (_, index) => index === 0 ? true : false))

    const navigateTo = (i: any) => {
        // navigate
        setActiveBtn(Array.from({ length: listButtonElement.length }, (_, index) => index === i ? true : false))
    }

    return (
        <Flex className="hidden-desktop z-1000" py='0.75rem' px='1.5rem' justify='space-between' bg='#fff' pos='fixed' bottom='0' left='0' right='0'>
